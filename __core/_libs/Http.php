...
class Http {
     ...
     * 返回的数组，对应于url请求的请求结果
     * @param array $urlArr         请求的url数组集
     * @param array   $header           请求时的head头
     * @param integer $oneLimit     超时时间，默认5秒
     * @return boolean || array
     */
    function curlM2($urlArr, $header = array(), $oneLimit = 5, $body=array()) {
        if (!$urlArr) {
            return false;
        }
        if (!is_array($urlArr)) {
        ...
        foreach ($urlArr as $key => $value) {
            $curls[$key] = curl_init($value);
            $setoption[CURLOPT_HTTPHEADER] = isset($header[$key]) ? $header[$key] : array();
            $setoption[CURLOPT_CUSTOMREQUEST] = isset($body[$key]) ? 'POST' : 'GET';
            $setoption[CURLOPT_POST] = isset($body[$key]) ? 1 : 0;
            $setoption[CURLOPT_POSTFIELDS] = isset($body[$key]) ? ($body[$key]) : null;
            curl_setopt_array($curls[$key], $setoption);
            curl_multi_add_handle($cm, $curls[$key]);
        }
