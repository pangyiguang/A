...
define('UPLOAD_ROOT', AHA_ROOT . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'upload');
...
if ($__all) {
    foreach ($__all as $__key => $__value) {
        ...
        if ($match) {
            array_shift($match);
            if (__getPreg($match, TIME_SATMP)) {//是否是要执行的文件
...
if ($__onFile) {
    Common::fileLog(DATA_ROOT . '/log/cron_index.log', '执行cron开始******************************' . date('Y-m-d H:i:s', TIME_SATMP) . '******************************', $__echo);
